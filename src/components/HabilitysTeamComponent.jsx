...
const HabilitysTeamComponent = ({ habilitys, img, name, description, link }) => {
    return (
        <motion.div whileHover={{scale:1.025,rotate:1}} className="bg-pllx-white p-8 lg:p-5 rounded-xl w-full flex flex-wrap items-center justify-between">
            <div className="flex flex-wrap justify-center lg:justify-start items-center gap-10 w-full md:w-full lg:w-1/2 xl:w-2/3">
                ...
                <div className="w-full lg:w-2/4 xl:w-2/3 text-pllx-black">
                    ...
                    <a className="flex justify-center items-center gap-2 w-[170px] bg-gradient-to-br from-pllx-theme to-pllx-accent rounded-full text-pllx-gray-dark px-3 py-2" target="_blank" href={link}>
                ...
                <div className=" flex flex-wrap gap-3 mt-2">
                    {
                        habilitys.map((hability, index) => (
                            <span className="bg-pllx-theme py-1 px-3 rounded-xl text-pllx-gray-dark font-medium font-switzer text-sm hover:drop-shadow-pllx-blue transition-all duration-300 cursor-pointer" key={index}>{hability}</span>
                        ))
                    }
                </div>
            </div>
